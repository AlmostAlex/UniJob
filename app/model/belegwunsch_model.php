...
class belegwunsch_model
{
    ...
    {
        $status = "offen";
        if ($statement = $this->dbh->prepare("UPDATE belegwunsch, thema SET belegwunsch.vorname = ?, belegwunsch.nachname = ?, 
                                                                            belegwunsch.email = ?, belegwunsch.studiengang = ?, belegwunsch.status = ?, belegwunsch.voraussetzung = ?, 
                                                                            belegwunsch.seminarteilnahme = ?, belegwunsch.wunschthema1 = ?, belegwunsch.wunschthema2 = ?, belegwunsch.wunschthema3 = ?, 
                                                                            belegwunsch.fachsemester = ?, belegwunsch.credits = ?, belegwunsch.punkte = ? WHERE belegwunsch.matrikelnummer = ? AND belegwunsch.wunschthema1 = thema.thema_id AND thema.modul_id = (SELECT modul_id FROM thema WHERE thema_id = ?)"))
        {
            $statement->bind_param('sssssssiiiiiiii', $vorname, $nachname, $email, $studiengang, $status, $voraussetzungen, $seminarteilnahme, $wunschthema1, $wunschthema2, $wunschthema3, $fachsemester, $credits, $punkte['gesamt'], $matrikelnummer, $wunschthema1);
            $statement->execute();
        } else {
            $error = $this->dbh->errno . ' ' . $this->dbh->error;
